...
# split the images from the strip
class Database_manupluations():
    ...
    ### import  directory to raw_images table
    def import_to_raw_image_from_local(self):
        ...
        for i in range(1,20, 1):
            try:
                img = open(r'D:\Final DMDD Project\raw_image\{}.jpg'.format(i), 'rb')
            ...
            insert_command = """INSERT INTO raw_table(img_id,image,image_date) VALUES (%s,%s,%s)"""
            ...
            if self.start_date == 31 and self.start_month in self.Months_list_2:
                print("entering into 30")
                self.start_month = self.start_month + 1
                self.start_date = 1
            if self.start_date == 29 and self.start_month == 2:
                print("entering into 28")
                self.start_month = self.start_month + 1

    ### raw_image_ to split_image
    def raw_image_to_split_image(self):
        try:
            ...
            for i in range(1, 20, 1):
                query = "SELECT image FROM raw_table WHERE img_id= {}".format(i)
                ...
                for key, value in cropped_box.items():
                    ...
                    cropped_image.save(r'D:\Final DMDD Project\split_images\{}_{}.png'.format(i, key))

        except Error as e:
            print('connection error {}'.format(e))

    #### save into the split_images table
    def store_split_images_to_db(self):
        ...
        for i in range(1, 20, 1):
            for j in range(1, 4, 1):
                    img = open(r'D:\Final DMDD Project\split_images\{}_{}.png'.format(i, j), 'rb')
                    formated_img = img.read()
...
db = Database_manupluations()
#db.import_to_raw_image_from_local()
#db.raw_image_to_split_image()
db.store_split_images_to_db()
